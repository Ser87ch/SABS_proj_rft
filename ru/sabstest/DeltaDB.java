...
public class DeltaDB {
	...
	try {
		...
		ResultSet rs = db.st.executeQuery("select * from " + table
		...
		while (rs.next()) {
			...
			if (!(rsMetaData.getColumnName(i).equals("action"))
				&& !(rsMetaData.getColumnName(i)
					.equals("changedate"))) {
			    Element col = doc.createElement("column");
			    col.setAttribute("name",
				    rsMetaData.getColumnName(i));
			    String s = rs.getString(i) != null ? rs
				    .getString(i) : "";
		...
		Log.msg("Таблица с измениями в БД '" + table
			+ "' записана в XML.");
	    }

	    XML.createXMLFile(doc, Settings.testProj + "\\tests\\"
		    + Settings.folder + "\\output\\" + num + "\\deltadb"
		    + String.format("%03d", count) + ".xml");
	    Log.msg("XML с изменениями в БД " + Settings.testProj + "\\tests\\"
		    + Settings.folder + "\\output\\" + num + "\\deltadb"
		    + String.format("%03d", count) + ".xml" + " создан.");

	    // XML.validate(Settings.testProj +
	    // "XMLSchema\\output\\deltadb.xsd",Settings.testProj + "\\tests\\"
	    // + Settings.folder + "\\output\\" + filename);
	...
	try {
	    ...
	    if (rs.next()) {
		s = "if  exists (select * from dbo.sysobjects where id = object_id('"
			+ t.name
			+ "_log'))\r\n"
			+ "drop table "
			+ t.name
		...
		do {
		    ...
		    if (rs.getString("tname").equals("char")
			    || rs.getString("tname").equals("varchar")
			    || rs.getString("tname").equals("nchar")
			    || rs.getString("tname").equals("nvarchar"))
			s = s + "(" + rs.getString("length") + ")";
		    else if (rs.getString("tname").equals("decimal"))
			s = s + "(" + rs.getString("xprec") + ", "
				+ rs.getString("xscale") + ")";
		    s = s + ",\r\n";
	...
	try {
	    ...
	    ResultSet rs = db.st.executeQuery(s);
	    if (rs.next()) {
		String drop = "if  exists (select * from dbo.sysobjects where id = object_id('"
			+ t.name
			+ "_ins_trg'))\r\n drop trigger "
			+ t.name
			+ "_ins_trg\r\n";

		s = "create trigger dbo." + t.name + "_ins_trg on dbo."
			+ t.name + "\r\n" + "for insert\r\n" + "as\r\n"
	...
	try {
		...
		s = "create trigger dbo." + t.name + "_del_trg on dbo."
			+ t.name + "\r\n" + "for delete\r\n" + "as\r\n"
			+ "begin\r\n insert into " + t.name + "_log\r\n"
			+ "(\r\n";
	...
	try {
		...
		s = "create trigger dbo." + t.name + "_upd_trg on dbo."
			+ t.name + "\r\n" + "for update\r\n" + "as\r\n"
			+ "begin\r\n";
		...
		do {
		    a = a + rs.getString("cname") + ", ";
		    b = b + "i." + rs.getString("cname") + " = d."
			    + rs.getString("cname") + " and ";
		} while (rs.next());
		...
		String prkeysql = "Select o2.name As TableName, o.name As PrimaryKeyName, tc.name As ColumnName\r\n"
			...
			+ "Where o.xtype = 'PK' And o2.xtype = 'U' and o2.name = '"
			+ t.name + "'\r\n";
		...
		s = s + "insert into " + t.name + "_log\r\n" + "(\r\n" + a
			+ "changedate, action\r\n" + ")\r\n" + "select " + a
			+ "getdate(), 'ud'\r\n" + "from Deleted d\r\n"
			+ "where not exists(select 1 from Inserted i where "
			+ b + ")\r\n" + "insert into " + t.name + "_log\r\n"
			+ "(\r\n" + a + "changedate, action\r\n" + ")\r\n"
			+ "select " + a + "getdate(), 'ui'\r\n"
			+ "from Inserted i\r\n"
    ...
    /**
     * создает таблицы и тригера для отслеживания изменений в БД для таблиц и
     * колонок, указанных в настройках
     */
	...
	try {
	    ...
	    Log.msg("Таблицы для записи лога для таблиц " + toStr()
		    + " созданы.");
	    Log.msg("Триггера для записи лога для таблиц " + toStr()
		    + " созданы.");
	...
	try {
	    ...
	    String s = "declare cur cursor for  \r\n"
		    + "select name,id from dbo.sysobjects where xtype = 'u' and not (name like '%_log')and name in("
		    + DeltaDB.toStr()
		    + ")\r\n"
		    + "\r\n"
	    ...
	    Log.msg("Таблицы и триггера для записи лога для таблиц " + toStr()
		    + " удалены.");
	    db.close();
	} catch (Exception e) {
	    e.printStackTrace();
    ...
    /**
     * @return возращает в виде строки все таблицы, для которых записываются
     *         изменения
     */
	...
	Log.msg("XML с настройками для подсчета изменений в БД " + fl
		+ " создан.");

	// XML.validate(Settings.testProj +
	// "XMLSchema\\settings\\deltadb.xsd",fl);
	...
	for (int temp = 0; temp < nList.getLength(); temp++) {
		...
		s = (Nm.item(0)).getNodeValue();
		t.add(s);
	    }
	    tables.add(t);
	}
	Log.msg("XML с настройками для подсчета изменений в БД " + src
	...
	for (int i = 0; i < fset.length; i++)
	    if (!cmpDeltaDB(fset[i].getAbsolutePath(),
		    fssrc[i].getAbsolutePath()))
		return false;
    ...
    /**
     * @param et
     *            первый xml с изменениями в БД
     * @param src
     *            второй xml с изменениями в БД
	...
	private void readXML(String src) {
	    ...
	    // XML.validate(Settings.testProj +
	    // "XMLSchema\\output\\deltadb.xsd",src);
		...
		for (int i = 0; i < nlList.getLength(); i++) {
		    Element Elmnt = (Element) nlList.item(i);
		    Line l = new Line(
			    Integer.parseInt(Elmnt.getAttribute("id")),
			    Elmnt.getAttribute("action"));
			...
			if (!El.getAttribute("name").equals("changedate")
				&& !El.getAttribute("name").equals("action")) {
			    l.addCol(s);
			    if (i == 0)
	...
	private boolean contains(Table t) {
	    ...
	    Log.msgCMP("Таблица из эталона " + t.tm.name
		    + " не найдена в изменениях БД.");
	    return false;
	}
    }
	...
	private boolean contains(Line ln) {
		...
		if (iterLn.next().equals(ln))
		    return true;
	    Log.msgCMP("Строка с id " + Integer.toString(ln.i)
		    + " из эталона не найдена в таблице " + tm.name + " .");
	    return false;
	}
    }
