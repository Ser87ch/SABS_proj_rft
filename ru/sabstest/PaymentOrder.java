	...
	public void insertIntoDB(int idPacet, int pEDNo, Date pacDate, String pAuthor, String filename) 
	{
		...
		try
		{
			...
			String query = "INSERT INTO [sabs_zapd].[dbo].[UFEBS_Epd]\r\n" + 
					...
					"null, " + DB.toString(accDocNo) + ", " + DB.toString(accDocDate) + ", " + DB.toString(paytKind) + ", " + DB.toString(sum) + ", " + DB.toString(payer.inn) + ", " + DB.toString(payer.name) + ", " + DB.toString(payer.personalAcc) + ", " + DB.toString(payer.bic) + ", " + DB.toString(payer.correspAcc) + ",\r\n" + 
