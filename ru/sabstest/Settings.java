...
public class Settings {
	...
	try {
	    ...
	    ResultSet rs = db.st
		    .executeQuery("select VALUE from dbo.XDM_CONF_SETTINGS where ID_PARAM = 1004");
	    rs.next();
	    ...
	    rs = db.st
		    .executeQuery("select X.BIK as BIK, X.NAME AS NAME, isnull(B.KSNP,'') as KSNP, B.RKC RKC from dbo.XDM_DEPARTMENT X inner join dbo.BNKSEEK B on B.NEWNUM = X.BIK");
	    rs.next();
	    ...
	    // ", имя ПУ: "+Settings.mainwindowname);

	    rs = db.st
		    .executeQuery("SELECT top 1 OPER_DATE as dt FROM XDM_OPERDAY_PROP WHERE VALUE = 0");
	    rs.next();
	    Settings.operDate = rs.getDate("dt");
	    // DM
	    Log.msg("Дата опер.дня: "
		    + new SimpleDateFormat("dd.MM.yyyy")
			    .format(Settings.operDate));
		...
		Settings.datafolder = Settings.testProj + "data\\" + filename
			+ "\\";
		Log.msg("Данные для теста находятся в " + Settings.testProj
			+ "data\\" + filename + "\\");
	    }
	    db.close();
	} catch (Exception e) {
	...
	// XML.validate(Settings.testProj + "XMLSchema\\settings\\general.xsd",
	// fl);
    }
	...
	// "XMLSchema\\settings\\general.xsd",src);
	...
	public static void createXML() {
	    createXML(Settings.testProj + "settings\\" + Settings.pervfolder
		    + "\\gendoc.xml");
	}
	...
	public static void createXML(String fl) {
	    ...
	    XML.createXMLFile(doc, fl);
	    Log.msg("XML с настройками для генерации входящих документов " + fl
		    + " создан.");
	    // XML.validate(Settings.testProj +
	    // "XMLSchema\\settings\\gen\\gendoc.xsd", fl);
	}
	...
	public static void readXML(String src) {
	    // XML.validate(Settings.testProj +
	    // "XMLSchema\\settings\\gen\\gendoc.xsd",src);
	    ...
	    Log.msg("XML с настройками для генерации входящих документов "
		    + src + " загружен в программу.");
	}
    }
	...
	public static void createXML(String fl) {
	    ...
	    // XML.validate(Settings.testProj +
	    // "XMLSchema\\settings\\login.xsd",fl);
	}
	...
	public static void readXML(String src) {
	    // XML.validate(Settings.testProj +
	    // "XMLSchema\\settings\\login.xsd",src);

	    Element root = XML.getXMLRootElement(src);

	    Element login = (Element) root.getElementsByTagName("pervvod")
		    .item(0);
	    pervvod = new LoginInfo(XML.getChildValueString("user", login),
	    ...
	    contrvvod = new LoginInfo(XML.getChildValueString("user", login),
		    XML.getChildValueString("pwd", login),
		    XML.getChildValueString("sign", login),
		    XML.getChildValueString("key", login));

	    login = (Element) root.getElementsByTagName("formes").item(0);
	    formes = new LoginInfo(XML.getChildValueString("user", login),
		    XML.getChildValueString("pwd", login),
		    XML.getChildValueString("sign", login),
		    XML.getChildValueString("key", login));
	    ...
	    eocontr = new LoginInfo(XML.getChildValueString("user", login),
		    XML.getChildValueString("pwd", login),
		    XML.getChildValueString("sign", login),
		    XML.getChildValueString("key", login));
	    ...
	    eootvet = new LoginInfo(XML.getChildValueString("user", login),
		    XML.getChildValueString("pwd", login),
		    XML.getChildValueString("sign", login),
		    XML.getChildValueString("key", login));
	    ...
	    eoadmin = new LoginInfo(XML.getChildValueString("user", login),
		    XML.getChildValueString("pwd", login),
		    XML.getChildValueString("sign", login),
		    XML.getChildValueString("key", login));

	    Log.msg("XML с настройками пользователей " + src
	...
	public static void createXML(String fl) {
	    ...
	    Log.msg("XML с настройками для генерации R-пакета " + fl
		    + " создан.");
	    // XML.validate(Settings.testProj +
	    // "XMLSchema\\settings\\sign.xsd",fl);
	}
	...
	public static void readXML(String src) {
	    // XML.validate(Settings.testProj +
	    // "XMLSchema\\settings\\sign.xsd",src);
	    ...
	    Log.msg("XML с настройками для генерации R-пакета " + src
		    + " загружен в программу.");
	}
