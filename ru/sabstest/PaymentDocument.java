...
abstract public class PaymentDocument implements Comparable<PaymentDocument> {
    ...
    public String systemCode; // признак системы обработки (заполняется если ЭД
			      // не составе пакета)
    ...
    /**
     * @param razd
     *            разделитель реквизитов
     * @param addShift
     *            добавляется ли Shift
	...
	str = Integer.toString(accDocNo)
		...
		+ razd + payer.bic + razd + payer.correspAcc + razd
		+ payer.personalAcc + razd + payee.bic + razd
		+ payee.correspAcc + razd + payee.personalAcc + razd + priority;
     ...
     *            файл
     */
    abstract public void insertIntoDbUfebs(int idPacet, int pEDNo,
	    Date pacDate, String pAuthor, String filename);
	...
	rootElement.setAttribute("EDDate",
	...
	accDoc.setAttribute("AccDocDate",
		new SimpleDateFormat("yyyy-MM-dd").format(accDocDate));
