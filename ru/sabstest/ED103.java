...
public class ED103 extends PaymentDocument {
    ...
    public Date docDispatchDate; // дата отсылки (вручения) плательщику
    ...
    public int acptSum; // сумма исходного расчетного документа, предъявленного
			// к акцепту
	...
	XML.setOptinalAttr(rootElement, "ReceiptDateCollectBank",
		receiptDateCollectBank);
	...
	if (doc.getLocalName().equals("ED103")
	    ...
	    receiptDateCollectBank = XML.getOptionalDateAttr(
		    "ReceiptDateCollectBank", doc);
	...
	try {
	    ...
	    String query = "INSERT INTO [dbo].[UFEBS_Epd]\r\n"
		    ...
		    + ", "
		    + DB.toString(receiptDateCollectBank)
		    + ",  "
		    + DB.toString(paytCondition)
		    + ", "
	...
	try {
	    ...
	    String query = "INSERT INTO [dbo].[epay_Epd]\r\n"
		    ...
		    + DB.toString(paytCondition)
		    + ", "
		    + DB.toString(acptTerm)
		    + ", "
		    + DB.toString(docDispatchDate)
		    + ",\r\n"
	...
	str = str
		+ ((paytKind.equals("2") || paytKind.equals("3")) ? " " + razd
			: "")
		+ razd
		+ ((paytKind.equals("2") || paytKind.equals("3")) ? "+{TAB}{ExtLeft}"
			: "") + (paytKind.equals("2") ? "{ExtLeft}" : "")
		+ ((paytKind.equals("2") || paytKind.equals("3")) ? razd : "");

	str = str + payer.bic + razd + payer.correspAcc + razd
		+ payer.personalAcc + razd + payer.inn + razd
		+ (addShift ? "+{ExtEnd}" : "") + payer.name + razd + payee.bic
		+ razd + payee.correspAcc + razd + payee.personalAcc + razd
		+ payee.inn + razd + (addShift ? "+{ExtEnd}" : "") + payee.name
		+ razd + priority + razd + tax.drawerStatus;
	...
	if (!is113Vvod)
	    str = str + razd + Integer.toString(acptTerm) + razd
		    + new SimpleDateFormat("ddMMyyyy").format(docDispatchDate)
		    + razd
		    + new SimpleDateFormat("ddMMyyyy").format(maturityDate)
		    + razd
	    ...
	    str = str + Integer.toString(acptTerm) + razd
		    + new SimpleDateFormat("ddMMyyyy").format(docDispatchDate)
		    + razd + razd + razd;
	return str;
    }
