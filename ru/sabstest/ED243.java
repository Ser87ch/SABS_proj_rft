...
public class ED243 extends Packet implements Generate<Element>, ReadED {
	...
	try {
	    ...
	    // String query =
	    // "INSERT INTO [dbo].[epay_ED243DEF]([ID_PACKET], [ID_ARM], [AccDocNo], [AccDocDate],\r\n"
	    // +
	    ...
	    // +
	    // "VALUES(null, '0', " + DB.toString(accDocNo) + ", " +
	    // DB.toString(accDocDate) + ",\r\n" +
	    // DB.toString(payeeAcc) + ", " + DB.toString(payerAcc) + ", " +
	    // DB.toString(sum) + ", " + DB.toString(payerName) + ", " +
	    // DB.toString(payeeName) + ", " + DB.toString(edDefineRequestText)
	    // + ",\r\n" +
	    ...
	    //
	    // int idED243 =
	    // Integer.parseInt(DB.selectFirstValueSabsDb("select max(ID_ED243DEF) from dbo.epay_ED243DEF"));
	    ...
	    String query = "INSERT [dbo].[UFEBS_Es201]([ID_PACET], [ID_DEPART], [EdNo], [EdDate],\r\n"
		    ...
		    + ", "
		    + DB.toString(edDefineRequestCode)
		    + ", null, null,\r\n"
		    + // " + DB.toString(idED243) +  "
		    "null, "
		    + DB.toString(iEdNo)
	...
	if (edAuthor.substring(0, 7).equals(Settings.bik.substring(2))) // особый
									// клиент
	    filename = "K";
	else
	    filename = "B";
	...
	filename = filename
		+ Integer.toString(Integer.parseInt(new SimpleDateFormat("MM")
			.format(edDate)), 36)
		+ Integer.toString(Integer.parseInt(new SimpleDateFormat("dd")
			.format(edDate)), 36);
	...
	Element ied = (Element) root.getElementsByTagNameNS("*", "OriginalEPD")
		.item(0);
	...
	Element ri = (Element) root.getElementsByTagNameNS("*",
		"EDDefineRequestInfo").item(0);
	...
	if (ri != null) {
	    ...
	    payerName = ri.getElementsByTagNameNS("*", "PayerName").item(0)
		    .getTextContent();
	    payeeName = ri.getElementsByTagNameNS("*", "PayeeName").item(0)
		    .getTextContent();
	    edDefineRequestText = ri
		    .getElementsByTagNameNS("*", "EDDefineRequestText").item(0)
		    .getTextContent();
		...
		for (int i = 0; i < nl.getLength(); i++)
		    edFieldList.add(((Element) nl.item(i))
			    .getElementsByTagNameNS("*", "FieldNo").item(0)
			    .getTextContent());
		...
		for (int i = 0; i < nl.getLength(); i++)
		    edReestrInfo.add(Integer.parseInt(((Element) nl.item(i))
			    .getElementsByTagNameNS("*", "TransactionID")
