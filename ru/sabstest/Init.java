...
public class Init {
	...
	try {
		...
		if (filename.substring(1, 6).equals("999999")) {
		    ...
		    DecimalFormat myFormat = new java.text.DecimalFormat(
			    "000000");
		    Settings.folder = filename.substring(0, 1)
			    + myFormat.format(new Integer(i));
		}
	    }
	    (new File(Settings.testProj + "tests\\" + Settings.folder)).mkdir();
	    Settings.fullfolder = Settings.testProj + "tests\\"
		    + Settings.folder + "\\";
	    ...
	    // DM
	    // Log.msg("Папка промежуточных данных для теста " +
	    // Settings.fullfolder + "input создана.");
	    // Log.msg("Папка исходящих данных для теста " + Settings.fullfolder
	    // + "output создана.");
	    Log.msg("Папка для входящих данных теста (" + Settings.fullfolder
		    + "input) создана.");
	    Log.msg("Папка для исходящих данных теста (" + Settings.fullfolder
		    + "output) создана.");

	    File[] files = new File(Settings.testProj + "settings\\test\\")
		    .listFiles();

	    for (File f : files) {
		String src = f.getName();

		(new File(Settings.fullfolder + "\\input\\"
			+ src.substring(0, 3))).mkdir();
		(new File(Settings.fullfolder + "\\output\\"
			+ src.substring(0, 3))).mkdir();
	    }
	...
	Log.createGen();
	Log.msg("Папка для теста новых данных " + Settings.testProj + "data\\"
		+ fld + " создана.");
	Log.msg("Папка входящих данных для теста " + Settings.testProj
		+ "data\\" + fld + "\\input создана.");
	Log.msg("Папка эталонных данных для теста " + Settings.testProj
		+ "data\\" + fld + "\\etalon создана.");
	...
	String filename = Settings.testProj + "data\\"
