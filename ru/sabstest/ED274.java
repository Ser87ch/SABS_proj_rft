...
public class ED274 extends Packet implements Generate<ED273>, ReadED {
	...
	annotation = root.getElementsByTagNameNS("*", "Annotation").item(0)
		.getTextContent();

	Element el = (Element) root.getElementsByTagNameNS("*", "InitialED")
		.item(0);
	...
	ED274CodeList.Code c = ED274CodeList.getInfoCode(source.pdList
		.get(ed273No).sum);
	if (c == null)
	    return false;
	infoCode = c.infoCode;
	...
	try {
	    ...
	    String query = "INSERT [dbo].[UFEBS_Es201]([ID_PACET], [ID_DEPART], [EdNo], [EdDate],\r\n"
		    ...
		    + ", "
		    + DB.toString(infoCode)
		    + ", "
		    + DB.toString(infoCode)
		    + ", "
		    + DB.toString(annotation)
		    + ",\r\n"
		    + "null, "
		    + DB.toString(iEdNo)
		    + ", "
		    ...
		    + DB.toString(edAuthor)
		    + ", "
		    + DB.toString(refEdDate)
		    + ", null, null,\r\n"
		    + DB.toString(refEdAuthor)
		    + ", '0', "
		    + DB.toString(refEdNo) + ", '0')";
