...
public class ED105 extends PaymentDocument {
	...
	accDoc.setAttribute("AccDocDate",
		new SimpleDateFormat("yyyy-MM-dd").format(ppAccDocDate));
	...
	if (doc.getLocalName().equals("ED105")) {
	    ...
	    Element pp = (Element) doc.getElementsByTagNameNS("*",
		    "PartialPayt").item(0);
	    ...
	    Element accDoc = (Element) pp.getElementsByTagNameNS("*", "AccDoc")
		    .item(0);
	...
	try {
	    ...
	    String query = "INSERT INTO [dbo].[UFEBS_Epd]\r\n"
		    ...
		    + ", null, null,\r\n"
		    + "null, "
		    + DB.toString(ppPaytNo)
		    + ", "
		    + DB.toString(ppTransKind)
		    + ", "
		    + DB.toString(ppAccDocNo)
		    + ", "
		    + DB.toString(ppAccDocDate)
		    + ", "
		    + DB.toString(ppSumResidualPayt)
		    + ", "
		    + DB.toString(receiptDate)
		    + ", "
		    + DB.toString(fileDate)
		    + ", "
		    + DB.toString(transContent)
		    + ", null, \r\n"
	...
	str = str
		...
		+ ppAccDocNo
		+ razd
		+ new SimpleDateFormat("ddMMyyyy").format(ppAccDocDate)
		+ razd
		+ Integer.toString(ppSumResidualPayt).substring(0,
			Integer.toString(ppSumResidualPayt).length() - 2)
		+ "."
		+ Integer.toString(ppSumResidualPayt).substring(
			Integer.toString(ppSumResidualPayt).length() - 2,
			Integer.toString(ppSumResidualPayt).length()) + razd
		+ transContent + razd +
