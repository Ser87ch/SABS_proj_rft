...
public class Client {
    ...
    public Client(String bik, String ks, String ls, String inn, String kpp,
	    String name) {
	this.bic = bik;
	this.correspAcc = ks;
	this.personalAcc = ls;
	...
	if (!personalAcc.equals("") && personalAcc != null) {
	    ...
	    if (bic.substring(6, 9).equals("000")
		    || bic.substring(6, 9).equals("001")
		    || bic.substring(6, 9).equals("002"))
		contrls = "0" + bic.substring(4, 6)
			+ personalAcc.substring(0, 8) + "0"
			+ personalAcc.substring(9, 20);
	    else
		contrls = bic.substring(6, 9) + personalAcc.substring(0, 8)
			+ "0" + personalAcc.substring(9, 20);
		...
		switch (k % 3) {
		case 0:
		    contr = (Character.getNumericValue(contrls.charAt(k)) * 7)
			    % 10 + contr;
		    break;
		case 1:
		    contr = (Character.getNumericValue(contrls.charAt(k)) * 1)
			    % 10 + contr;
		    break;
		case 2:
		    contr = (Character.getNumericValue(contrls.charAt(k)) * 3)
			    % 10 + contr;
		    break;
		}
	    ...
	    personalAcc = personalAcc.substring(0, 8) + Integer.toString(contr)
		    + personalAcc.substring(9, 20);
	}
    }
    ...
    /**
     * @param doc
     *            документ для создания елемента
     * @param elementName
    ...
    /**
     * @param cl
     *            элемент с реквизитами клиента считывает реквизиты клиента из
     *            элемента
     */
    ...
    /**
     * Создает нового клиента. @ - текущий БИК. В случае если в данных находится
     * sql запрос, он выполняется и в значения БИК подставляется либо первое
     * значение, либо указанное в row, в счет подставляется первое значение.
     * Если БИК текущий, реквизиты клиента берутся из базы.
     * 
     * @param clEl
     *            элемент с данными для генерации реквизитов клиентов
	...
	Element bicEl = (Element) clEl.getElementsByTagNameNS("*", "BIC").item(
		0);
	Element paEl = (Element) clEl
		.getElementsByTagNameNS("*", "PersonalAcc").item(0);
	...
	if (bic.equals(Settings.bik) && !personalAcc.equals("")) {
	    if (personalAcc.substring(0, 1).equals("3")
		    || personalAcc.substring(0, 1).equals("4")) {
		name = DB
			.selectFirstValueSabsDb("select c.nameshort from dbo.Account a\r\n"
		...
		name = DB
			.selectFirstValueSabsDb("select a.name_short from dbo.Account a where a.NUM_ACC = '"
				+ personalAcc + "'");
	} else {
	    correspAcc = DB
		    .selectFirstValueSabsDb("select top 1 isnull(KSNP,'') ksnp from dbo.BNKSEEK where NEWNUM = '"
			    + bic + "'");
    ...
    public static Client createClientFromBICPersonalAcc(String bic, String ls,
	    String uic, Sign sign1, Sign sign2) {
