...
public class Pack {
    ...
    /**
     * @param dest
     *            папка, содержимое которой копируется
     * @param src
     *            папка, в которую копируется
     */
	...
	File[] files = new File(Settings.datafolder + "input\\" + num)
		.listFiles();

	for (File fl : files)
	    if (fl.getName().length() >= 33)
		copyFile(fl.getAbsolutePath(), Settings.path + "post\\kPuI\\"
			+ fl.getName());
	    else
		copyFile(fl.getAbsolutePath(), Settings.path + "post\\kUfI\\"
			+ fl.getName());
    }
	...
	if (isVER) {
	    ...
	    for (File fl : files)
		copyFile(fl.getAbsolutePath(), Settings.fullfolder
	...
	for (File fl : files)
	    Pack.copyFile(fl.getAbsolutePath(), Settings.fullfolder
