...
public class ED104 extends PaymentDocument {
	...
	str = str + payer.bic + razd + payer.correspAcc + razd
		+ payer.personalAcc + razd + payer.inn + razd + payer.kpp
		+ razd + (addShift ? "+{ExtEnd}" : "") + payer.name + razd
		+ payee.bic + razd + payee.correspAcc + razd
		+ payee.personalAcc + razd + payee.inn + razd + payee.kpp
		+ razd + (addShift ? "+{ExtEnd}" : "") + payee.name + razd
	...
	if (!is114Vvod)
	    str = str
		    ...
		    + razd
		    + ((paytKind.equals("2") || paytKind.equals("3")) ? ""
			    : razd) + razd;
	else
	    str = str + new SimpleDateFormat("ddMMyyyy").format(chargeOffDate)
		    + razd + razd + razd;
	return str;
    }
}
